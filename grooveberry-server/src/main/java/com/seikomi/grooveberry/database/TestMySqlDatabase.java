...
public class TestMySqlDatabase {
	private static final Logger LOGGER = LoggerFactory.getLogger(TestMySqlDatabase.class);
	private static SongDAO songDAO = new SongDAO();
	private static PlaylistDAO playlistDAO = new PlaylistDAO();

	public static void main(String[] args) throws SQLException {
		Statement statement = ConnectionMySQLDatabase.getConnection().createStatement();
		statement.executeUpdate("RUNSCRIPT FROM '"
				+ new File("D:\\Perso\\workspace\\GrooveBerry\\grooveberry-server\\src\\main\\resources\\sql\\init.sql")
				+ "'");
		...
		ConnectionMySQLDatabase.getConnection().close();
